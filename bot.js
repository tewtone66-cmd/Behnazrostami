...
if (!token) { console.log('[telegram] BOT_TOKEN is not set; Telegram bot is disabled.'); module.exports = null; } else {
  ...
  async function forwardReceipt(m) {
    if (!adminChatId) { console.error('[telegram] ADMIN_CHAT_ID is not set; owner must message the bot once.'); return false; }
    ...
    if(m.photo) { await telegram('sendPhoto',{...common,photo:m.photo[m.photo.length-1].file_id}); return true; }
    if(m.document) { await telegram('sendDocument',{...common,document:m.document.file_id}); return true; }
    return false;
  }
  async function handleMessage(m) {
    ...
    if(m.photo||m.document) {
      ...
      return send(chatId,delivered?'📤 رسید دریافت شد و برای مدیر ارسال شد. بعد از بررسی نتیجه به شما اعلام می‌شود.':'📤 رسید دریافت شد، اما مدیر هنوز بات را فعال نکرده است. لطفاً بعداً دوباره تلاش کنید.');
    }
  ...
  async function handleCallback(q) {
    ...
    // Customer actions must remain public. Only receipt-management actions below are admin-only.
    ...
    return send(chatId,'گزینه موردنظر پیدا نشد.');
  }
